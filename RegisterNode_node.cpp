...
public:
	void nmea_callback(const msgs::nmea::ConstPtr& nmea_msg){
		...
		if(!nmea_msg->type.compare("GPGGA") && nmea_msg->valid == true) {
			...
			double latitude =  nmea_latlon( (char* ) nmea_msg->data[1].c_str() );
			double longitude = nmea_latlon( (char* ) nmea_msg->data[3].c_str() );
			double altitude = atof( nmea_msg->data[3].c_str() );
